  ...
  useEffect(() => {
    if (state?.scholarship) {
      const fetchScholarshipDetails = async () => {
        try {
          const response = await fetch(`https://172.31.41.116:5000/api/scholarships/${state.scholarship._id}`);
...
const Form = ({ scholarshipName, setIsApplied }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://172.31.41.116:5000/api/form/submit', data, {
        headers: {
