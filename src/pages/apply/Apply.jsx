  ...
  useEffect(() => {
    if (state?.scholarship) {
      const fetchScholarshipDetails = async () => {
        try {
          const response = await fetch(`https://shradha.onrender.com/api/scholarships/${state.scholarship._id}`);
...
const Form = ({ scholarshipName, setIsApplied }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://shradha.onrender.com/api/form/submit', data, {
        headers: {
