    ...
    const fetchUserForms = async () => {
      try {
        ...
        if (user && user.role === 2) { // Assuming role 2 is for state admin
          ...
          let url = `https://172.31.41.116:5000/api/forms/state/${stateEncoded}?status=${statusFilter}`;
          if (toggle === 'other') {
            url = `https://172.31.41.116:5000/api/forms/otherstate/${stateEncoded}?status=${statusFilter}`;
          }
