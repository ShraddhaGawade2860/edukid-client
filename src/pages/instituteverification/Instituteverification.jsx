...
const InstituteVerification = () => {
    ...
    useEffect(() => {
        ...
        const fetchVerificationRequests = async () => {
            try {
                const response = await axios.get('https://35.160.120.126:5000/api/admin/verification/requests', {
                    params: { state: state }
                });
    ...
    const approveInstitute = async (id) => {
        try {
            await axios.put(`https://35.160.120.126:5000/api/admin/approve/${id}`);
    ...
    const rejectInstitute = async (id) => {
        try {
            await axios.put(`https://35.160.120.126:5000/api/admin/reject/${id}`);
                        ...
                        verificationRequests.map((req) => (
                            <div key={req._id} className={styles['verification-request-box']}>
                                ...
                                <p>
                                    ...
                                    <a href={`https://35.160.120.126:5000/${req.instituteCertificate}`} target="_blank" rel="noopener noreferrer">View</a>
                                </p>
                                <p>
                                    <strong>Accreditation Certificate:</strong> 
                                    <a href={`https://35.160.120.126:5000/${req.accreditationCertificate}`} target="_blank" rel="noopener noreferrer">View</a>
                                </p>
                                <p>
                                    <strong>Affiliation Certificate:</strong> 
                                    <a href={`https://35.160.120.126:5000/${req.affiliationCertificate}`} target="_blank" rel="noopener noreferrer">View</a>
                                </p>
