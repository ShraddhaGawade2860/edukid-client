...
const EditInstitute = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      setImagePreview(user.profileImage ? `http://3.110.2.125:5000/uploads/${user.profileImage}` : '');
    }
  }, [user]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`http://3.110.2.125:5000/api/users/${user.id}`, formDataToSend, {
        headers: {
