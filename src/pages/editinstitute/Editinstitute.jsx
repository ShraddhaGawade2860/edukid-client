...
const EditInstitute = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      setImagePreview(user.profileImage ? `https://35.160.120.126:5000/uploads/${user.profileImage}` : '');
    }
  }, [user]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`https://35.160.120.126:5000/api/users/${user.id}`, formDataToSend, {
        headers: {
