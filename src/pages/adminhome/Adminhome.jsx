...
const AdminHome = () => {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        const response = await fetch(`http://172.31.41.116:5000/api/admincount/state/${state}/counts`);
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchPendingForms = async () => {
      try {
        const response = await axios.get(`http://172.31.41.116:5000/api/admincount/state/${state}?status=pending`);
  ...
  useEffect(() => {
    const fetchScholarshipCount = async () => {
      try {
        const response = await fetch('http://172.31.41.116:5000/api/scholarshipcount/count');
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        const data = await response.json();
